...
```{r faces, echo=FALSE, fig.align = 'center'}
...
Assuming that face 7 is fixed, the vertices of the common edge are at corners 1 and 4, and in the table the opposite corners for those are 2 and 3 (by row 2). So if face 11 can be rotated in 3d so that vertex 13 is in corner 2 instead of corner 1, and vertex 11 is in corner 3 instead of corner 2, then the two faces will be correctly aligned on the plane.

The following figure shows all the possible 3d rotations of face 11.
...
So evidently the d8 element named "a3" is the rotation that is needed, and the *layoutsquares* package functions seem to agree:
...
```
The documentation for d8 describes the operations in a way that follows what appears in Figure 3.
